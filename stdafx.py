...
class GuiTable():
    MainWindow = tk.Tk()
    m_pXYWidget = tk.Canvas(MainWindow, relief= 'sunken', background= 'Black', bd =3, width= 730, height= 400)
    m_pXZWidget = tk.Canvas(MainWindow, relief= 'sunken', background= 'Black', bd = 3, width= 730, height= 400)
    m_pYZWidget = tk.Canvas(MainWindow, relief= 'sunken', background= 'Black', bd = 3, width= 800, height = 400)
    m_pCamWidget = tk.Canvas(MainWindow, relief= 'sunken', bg= 'DarkGray', bd= 0.5, width= 800, height= 400, borderwidth= 3)
...
def PrintGridViewType(m_pGridDummy):
    if m_pGridDummy == GuiTable.m_pXYWidget:
        m_pGridDummy.create_text(25, 25, text= "XY", font= ("Arial", 18), fill = "white")
...
def InitRender():
   ...
   RenderMsgBox = MSGBOX.askokcancel('Camera Window Construction', 'Camwnd 3D Viewport Initalized', icon= 'info')
   for i in range(0):
...
def GridDrawRectangle(m_pGridDummy):
    m_pGridDummy.create_rectangle(10, 10, 300, 300, fill= "", outline="yellow", width= 2)
